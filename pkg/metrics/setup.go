...
type Metrics struct {
	...
	// Custom metrics owned by this service
...
func NewMetrics(cfg Config) *Metrics {
	...
	// Define metrics using helpers
	m.requestsTotal = createCounterVec(cfg.ServiceName, "requests_total", "Total number of processed requests", []string{"status"})
	m.requestDuration = createHistogramVec(cfg.ServiceName, "request_duration_seconds", "Duration of HTTP requests in seconds", []string{"endpoint"}, prometheus.DefBuckets)
	m.cpuUsageGauge = createGaugeVec(cfg.ServiceName, "cpu_usage_percent", "Current CPU usage percentage per core", []string{"core"})
